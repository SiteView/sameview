...
export function placeCaretAtEnd(el) {
    el.focus();
    el.selectionStart = el.value !== undefined ? el.value.length : el.innerHTML.length;
    el.selectionEnd = el.value !== undefined ? el.value.length : el.innerHTML.length;
...
export function getEmojiMessage(message) {
    const msg = message.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, function (match, capture) {
        const in1 = match.indexOf('alt='), in2 = match.indexOf('class=');
        const emojiAlias = match.substring(in1 + 4, in2 -1);
        // console.log(emojiAlias.replace(/"/g, ""));
        return emojiAlias.replace(/"/g, "");
    });
    return msg;
}
